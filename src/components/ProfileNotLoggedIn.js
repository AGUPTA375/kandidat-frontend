...
export default function ProfileNotLoggedIn (props) {
  ...
  useEffect(() => {
    if (sign) {
      if (!GDPR) {
        ...
        signup({ name: nameRegister, phone_number: phoneRegister.toString(), password: passwordRegister, picture: imgb64, business })
          .then((data) => {
            if (data[0] === 201) {
              Alert.alert(
                'Account created!',
  ...
  return (
    <View style={styles.root}>
      ...
      <TouchableOpacity
        style={[styles.button, { backgroundColor: '#650001' }]}
